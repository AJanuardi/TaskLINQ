using System;
using Newtonsoft.Json;
using System.Collections.Generic;
using System.Collections;
using System.Linq;

namespace Json
{
    class Program
    {
        static void Main(string[] args)
        {
            string json = @"[
  {
    ""order_id"": ""SO-921"",
    ""created_at"": ""2018-02-17T03:24:12"",
    ""customer"": { ""id"": 33, ""name"": ""Ari"" },
    ""items"": [
      { ""id"": 24, ""name"": ""Sapu Lidi"", ""qty"": 2, ""price"": 13200 }, 
      { ""id"": 73, ""name"": ""Sprei 160x200 polos"", ""qty"": 1, ""price"": 149000 }
    ]
  },
  {
    ""order_id"": ""SO-922"",
    ""created_at"": ""2018-02-20T13:10:32"",
    ""customer"": { ""id"": 40, ""name"": ""Ririn"" },
    ""items"": [
      { ""id"": 83, ""name"": ""Rice Cooker"", ""qty"": 1, ""price"": 258000 },
      { ""id"": 24, ""name"": ""Sapu Lidi"", ""qty"": 1, ""price"": 13200 }, 
      { ""id"": 30, ""name"": ""Teflon"", ""qty"": 1, ""price"": 190000 }
    ]
  },
  {
    ""order_id"": ""SO-923"",
    ""created_at"": ""2018-02-28T15:20:43"",
    ""customer"": { ""id"": 33, ""name"": ""Ari"" },
    ""items"": [
      { ""id"": 303, ""name"": ""Pematik Api"", ""qty"": 1, ""price"": 12000 }, 
      { ""id"": 49, ""name"": ""Panci"", ""qty"": 2, ""price"": 70000 }
    ]
  },
  {
    ""order_id"": ""SO-924"",
    ""created_at"": ""2018-03-02T14:30:54"",
    ""customer"": { ""id"": 40, ""name"": ""Ririn"" },
    ""items"": [
      { ""id"": 986, ""name"": ""TV LCD 40 inch"", ""qty"": 1, ""price"": 6000000 }
    ]
  },
  {
    ""order_id"": ""SO-925"",
    ""created_at"": ""2018-03-03T14:52:22"",
    ""customer"": { ""id"": 33, ""name"": ""Ari"" },
    ""items"": [
      { ""id"": 1033, ""name"": ""Nintendo Switch"", ""qty"": 1, ""price"": 4990000 }, 
      { ""id"": 2003, ""name"": ""Macbook Air 11 inch 128 GB"", ""qty"": 1, ""price"": 12000000 },
      { ""id"": 23, ""name"": ""Pocari Sweat 600ML"", ""qty"": 5, ""price"": 7000 }
    ]
  },
  {
    ""order_id"": ""SO-926"",
    ""created_at"": ""2018-03-05T16:23:20"",
    ""customer"": { ""id"": 58, ""name"": ""Annis"" },
    ""items"": [
      { ""id"": 24, ""name"": ""Sapu Lidi"", ""qty"": 3, ""price"": 13200 }
    ]
  }
]";
            User obj = new User();
            var ObjekList = JsonConvert.DeserializeObject<List<User>>(json);
            var CustomerList = JsonConvert.DeserializeObject<List<Customer>>(json);

            Console.WriteLine("==========================");
            Console.WriteLine("Pembelian pada bulan Februari");
            var result1 = ObjekList.Where(i => (i.Created).Month == 2);
            foreach (var x in result1)
            {
                Console.WriteLine(x.Customer.Name);
            }

            Console.WriteLine("==========================");
            Console.WriteLine("Pembelian Ari");
          try
           {
               var result2 = ObjekList.Where(i => (i.Customer.Name).Contains("Ari"));
               foreach(var i in result2)
               {
                   Console.WriteLine(i.Id);
               }
           }
          catch (Exception e)
           {
               Console.WriteLine("Customer Tidak Ditemukan");
               throw;
           }

          Console.WriteLine("==========================");
          Console.WriteLine("Total Pembelian Ari");
          try
           {
               int sum = 0;
               var result2 = ObjekList.Where(i => (i.Customer.Name).Contains("Ari"));
               foreach(var i in result2)
               {
                   foreach(var x in i.ItemList)
                   {
                     sum += ((x.Price)*(x.Quantity));
                   }
               }

               Console.WriteLine(sum);
           }
          catch (Exception e)
           {
               Console.WriteLine("Customer Tidak Ditemukan");
               throw;
           }

          Console.WriteLine("==========================");
          Console.WriteLine("Total Pembelian dibawah 300.000");
          try
           {
               int a = 0;
               int b = 0;
               int c = 0;
               var result3 = ObjekList.Where(i => (i.Customer.Name).Contains("Ari"));
               foreach(var i in result3)
               {
                   foreach(var x in i.ItemList)
                   {
                     a += ((x.Price)*(x.Quantity));
                   }
               }
               var result4 = ObjekList.Where(i => (i.Customer.Name).Contains("Annis"));
               foreach(var i in result4)
               {
                   foreach(var x in i.ItemList)
                   {
                     b += ((x.Price)*(x.Quantity));
                   }
               }
               var result5 = ObjekList.Where(i => (i.Customer.Name).Contains("Ririn"));
               foreach(var i in result5)
               {
                   foreach(var x in i.ItemList)
                   {
                     c += ((x.Price)*(x.Quantity));
                   }
               }
               
               if (a < 300000)
               {
                 Console.WriteLine("Ari");
               }
               else if (b < 300000)
               {
                 Console.WriteLine("Annis");
               }
               if (c < 300000)
               {
                 Console.WriteLine("Ririn");
               }
           }
          catch (Exception e)
           {
               Console.WriteLine("Customer Tidak Ditemukan");
               throw;
           }
        }
    }


    class Customer
    {
        [JsonProperty("id")]
        public int Id { get; set; }

        [JsonProperty("name")]
        public string Name { get; set; }
    }

    class User
    {
        [JsonProperty("order_id")]
        public string Id { get; set; }

        [JsonProperty("created_at")]
        public DateTime Created { get; set; }

        [JsonProperty("customer")]
        public Customer Customer { get; set; }

        [JsonProperty("items")]
        public List<items> ItemList { get; set; } = new List<items>();
    }

    public class items
    {
        [JsonProperty("id")]
        public int Id { get; set; }

        [JsonProperty("name")]
        public string Name { get; set; }

        [JsonProperty("qty")]
        public int Quantity { get; set; }

        [JsonProperty("price")]
        public int Price { get; set; }
    }
}